...
const std::string TEXTURE_PATH = "textures/lunar-rock-bl-1/lunar-rock-albedo.png";
...
// Used to control what severities of messages should be displayed
VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverities[4] = {
...
uint8_t minimumMessageSeverity = 0;

// If the program is in debug mode, then enableValidationLayers will be set to true and all validation layers will be enabled throughout the program to assist with debugging
// Otherwise, if it's not then all validation layers will be disabled to boost performance
#ifdef NDEBUG
...
// Proxy function to create the debug messenger object as the base one isn't automatically loaded 
...
// Proxy function to destroy the debug messenger object as the base one isn't automatically loaded 
...
// Struct to store the queue indices
struct QueueFamilyIndices {
	std::optional<uint32_t> graphicsFamily;
	std::optional<uint32_t> presentFamily;

	// Only returns true if both variables have a value
	bool isComplete() {
		return graphicsFamily.has_value() && presentFamily.has_value();
	}
};

// Struct containing information regarding swap chain properties to check device support
...
private:
	...
	void initWindow() {
		...
		window = glfwCreateWindow(WIDTH, HEIGHT, "Striatum", nullptr, nullptr);
		glfwSetWindowUserPointer(window, this);
		glfwSetFramebufferSizeCallback(window, framebufferResizeCallback);
	}

	// Callback triggered whenever the application is resized
	static void framebufferResizeCallback(GLFWwindow* window, int width, int height) {
		auto app = reinterpret_cast<HelloTriangleApplication*>(glfwGetWindowUserPointer(window));
		app->framebufferResized = true;
	}

	// Initialize everything for the application to run
	void initVulkan() {
		createInstance();
		setupDebugMessenger();
		createSurface();
	...
	void createInstance() {
		// If program is in debug mode, then checks if validation layers are available
		if (enableValidationLayers && !checkValidationLayerSupport())
			throw std::runtime_error("Validation layers requested, but not available!");

		// Defines information regarding the application itself
		VkApplicationInfo appInfo{};
		appInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
		appInfo.pApplicationName = "Hello Triangle";
		appInfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
		appInfo.pEngineName = "Striatum";
		...
		// If in debug mode, configure the debug object to be able to check over the instance creation
		...
		/* 
		Debug function printing all available extensions
		...
		*/
		...
		// Debug function confirming all required extensions are available
		for (int i = 0; i < extensionCount; i++) {
			bool found = false;
			for (const auto& availableExtension : availableExtensions) {
				if (!strcmp(extensions[i], availableExtension.extensionName)) {
					found = true;
					break;
				}
			}
			...
			if (!found)
				std::cout << extensions[i] << " was not found" << std::endl;
		}

		// Attempt to create a vk instance and throw an error on failure
		if (vkCreateInstance(&createInfo, nullptr, &instance) != VK_SUCCESS)
			throw std::runtime_error("failed to create instance");
	}

	// If in debug mode, setup the debug messenger
	...
	// Creates a surface which links vulkan to the window system
	void createSurface() {
		if (glfwCreateWindowSurface(instance, window, nullptr, &surface) != VK_SUCCESS)
			throw std::runtime_error("Failed to create window surface");
	}

	// Pick a gpu that meets the application's requirements
	...
	void createLogicalDevice() {
		// Creating a struct with all info required to create a queue
		...
		// Specifices what device features will be used
		VkPhysicalDeviceFeatures deviceFeatures{};
		deviceFeatures.samplerAnisotropy = VK_TRUE;

		// Creating the logical device struct which is linked to the queue structs and the device features struct
		...
		// If project is in debug mode, enable validation layers for the logical device 
		if (enableValidationLayers) {
			createInfo.enabledLayerCount = static_cast<uint32_t>(validationLayers.size());
			createInfo.ppEnabledLayerNames = validationLayers.data();
		}
		...
		// Create the logical device, throwing an error if something goes wrong
		if (vkCreateDevice(physicalDevice, &createInfo, nullptr, &device) != VK_SUCCESS)
			throw std::runtime_error("Failed to create logical device!");

		// Retrieves and stores the graphics queue handle
	...
	// Creates the image views (they're views into the images like the ones stored in the swap chain by decribing details such as how to access the image eg: a 2d depth texture without mipmapping levels)
	...
	// Creates a layout for the descriptor set
	void createDescriptorSetLayout() {
		...
		std::array<VkDescriptorSetLayoutBinding, 3> bindings = { uboLayoutBinding, camPosLayoutBinding, samplerLayoutBinding };
	...
	void createGraphicsPipeline() {
		...
		// Culls back faces. In addition indices are listed counter clockwise to determine front faces
	...
	// Creates a frame buffer for each of the swap chain images
	...
	void createTextureImage() {
		...
		// Transitioned into VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL while generating mipmaps
	...
	// Creates a descriptor pool set up to have a descriptor for each frame in flight
	...
	// Creates the semaphores and fence required to sync the rendering
	...
	void populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
		...
		createInfo.messageSeverity = VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT; // Callback will be triggered for all debug messages except severity level "info"
		createInfo.messageType = VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT;
		createInfo.pfnUserCallback = debugCallback;
		createInfo.pUserData = nullptr;
	}

	// Checks if the given device meets the project's requirements
	...
	// Find a queue that meets the application's requirements
	...
	// Gets all required extensions, including a debug extension if in debug mode
	...
	// Checks if all required validation layers are available
	...
	// Callback for when debug messages are sent, only messages equal or higher in priority then the minimum are logged
	static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
		...
		if (messageSeverity >= messageSeverities[minimumMessageSeverity])
			std::cerr << "Validation layer: " << pCallbackData->pMessage << std::endl;

		return VK_FALSE;
	}

	// Finds all swap chain support that a given device has
	...
	// Finds the ideal format for the swap chain and if unavailable, just returns the first available option (ideally would rank formats and would choose best alterantive option)
	...
	// Finds the ideal present mode which is mailbox but falls back to fifo on failure as it's always available
	...
	// Calculates the extent of the swap chain in pixels (width and height previously defined was in screen cords)
	...
	// Helper function that reads a given file's contents into a char buffer
	...
	// Creates a shader module object from the given shader code
	...
	// Writes commands into a given command buffer
	...
	// Used to destroy old swapchain and objects depending on it
	...
	// Used to recreate the swapchain and the objects depending on it for events such as the window being resized
	...
	// Finds a memory type that suits the given usecase
	...
	// Creates a buffer with the given properties
	...
	// Copies buffer content between the src and dst buffer
	...
	// Transitions the given image between layouts
	...
	// Copies buffer content into image
	...
	// Tries to find a format with the required features that the selected physical device has
	...
	// Generates mipmaps - should be replaced to deal with issues caused by image formats not supporting linear blitting (Eg: using a library like stb_image_resize)
	...
	// Finds max number of samples the physical device supports
	...
	// Sets the window title to the current frame rate
	...
	void clearBuffer(VkBuffer buffer, VkDeviceSize offset = 0, VkDeviceSize size = VK_WHOLE_SIZE) {
		VkCommandBuffer commandBuffer = beginSingleTimeCommands();
		vkCmdFillBuffer(commandBuffer, buffer, offset, size, -1);
		endSingleTimeCommands(commandBuffer);
	}
	...
	// Prevents program from automatically closing
	void mainLoop() {
	...
	// Deletes initialized objects and free allocated memory when program is closed
	void cleanup() {
		cleanupSwapChain();
...
// Code entry point which initializes application
int main() {
